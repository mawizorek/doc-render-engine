...
RED READING THE SOURCE IS WHY THIS WORKS FIRST TIME. The previous attempt to
beat a Material rule in this engine -- the dark-mode blue link -- shipped
...
WARNING: THAT IS A TIE, NOT A STRUCTURAL FIX. There is nothing to delete here --
Material's rule is in its own stylesheet and stays there. base.css's ARMOUR
...
WARNING: THE `details` SELECTORS ARE DEFENSIVE AND I COULD NOT VERIFY THEM.
...
from . import state
from .markers import _colour, _token_sets
...
#: table's job.
#:
#: WARNING: `font-size` IS THE ONE VISIBLE CHANGE NOBODY ASKED FOR. Material
#: sets 0.64rem, which on this site is far below body text; eos's `fs-sm` is
#: 0.82rem against a 0.9rem body. Callouts get bigger and sit just under the
#: prose instead of looking like a footnote. That is the correct relationship
#: rather than the one that happened to exist, and it is stated here so it is
...
def build_css(report: bool = False) -> str:
    ...
    # variables -- so those are mapped in base.css and NOT restated here. One
    # surface, one address.
    out.append(
