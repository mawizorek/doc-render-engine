...
⚠️ **SCOPED, NOT GREENLIT.** 2026-08-04. Nothing built. Decision history: the **doc-render-engine (repo) — Decision Log** subpage in ClickUp.
...
**2. `.replace("`", "")` destroys code FENCES.** It exists to strip inline code spans, which is right for a cell. Run it over a document and ```` ``` ```` becomes an empty string — so a fenced block does not lose its formatting, **it stops being a block at all** and its contents merge into the body as prose. Silent, and catastrophic on any page carrying an example.
...
- Python names a group `(?P<text>…)`; JavaScript names it `(?<text>…)`. The engine's `_LINK` and `_MARK` patterns both use named groups, so **neither one is portable as written.**
- `cells._EM` uses the lookbehind `(?<![*\w])`. Safari shipped lookbehind in **16.4**; anything older throws on the pattern itself, not on the input.
- Order is load-bearing and is not in the pattern. `plain()` strips braces *before* links so `[x](@term:y){.cls}` degrades correctly. A consumer given four patterns and no order gets a different answer.
...
## Files and sizes (measured at HEAD, not estimated)
...
| `docrender/markers.py` | 16,241 B | ⚠️ **near the ceiling — expose `_TABLE` via a small accessor, add nothing else.** |
| `docrender/cells.py` | 9,660 B | **untouched** (see ruling 4). |
| `mkdocs.yml` | 5,580 B | two hook registrations. ⚠️ Its own comment records a hook that has been dead exactly this way. |

🔴 **Both new files go in their own modules. Do not append to `markers.py` (16,241 B) or `datatable.py`** — the Prism spec already carries that instruction.

⚠️ **Doc rot caught while measuring:** `prism/next-build-spec.md` states *"`docrender/datatable.py` is 16,410 B."* **At HEAD it is 14,885 B.** The `markers.py` figure in the same sentence is correct. One stale number, corrected here rather than left to be quoted again.
...
**Steps 1 and 2 can ship tonight and in either order.** Neither depends on the other, and neither depends on anything in Prism or git-grab.
